...
class Widget extends \WP_Widget {
				...
				'filter'  => array(
					...
					'items'       => array(
						...
						'post_ids'      => array(
							...
							'type'          => 'text',
							'input_attrs'   => array(
								'placeholder' => esc_attr_x( 'Comma separated list of post ids, i.e. 230,300', 'Placeholder text for post ids', 'display-post-types' ),
							),
							'hide_callback' => function() use ( $widget, $instance ) {
								return 'page' === $instance['post_type'];
							},
		...
		foreach ( $options as $option => $args ) {
			...
			foreach ( $items as $item => $attr ) {
				...
				switch ( $type ) {
					...
					case 'text':
						$optmar  = $this->label( $set, $label, false );
						$optmar .= sprintf( '<input class="widefat dpt-%1$s" name="%2$s" id="%3$s" type="text" value="%4$s" %5$s />', str_replace( '_', '-', $set ), $name, $id, is_array( $instance[ $set ] ) ? implode( ',', $instance[ $set ] ) : esc_attr( $instance[ $set ] ), $inputattr );
					...
					case 'color':
						$optmar  = $this->label( $set, $label, false );
						$optmar .= sprintf( '<input class="dpt-color-picker" data-alpha-enabled="true" name="%1$s" id="%2$s" type="text" value="%3$s" />', $name, $id, esc_attr( sanitize_text_field( $instance[ $set ] ) ) );
						break;
					case 'taxonomy':
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['query_tax_relation'] = isset( $new_instance['query_tax_relation'] )
			? \Display_Post_Types\Frontend\Inc\Query_Schema::normalize_relation( $new_instance['query_tax_relation'] )
			: 'AND';
		$instance['query_tax_clauses']  = ( $instance['post_type'] && 'page' !== $instance['post_type'] && ! empty( $new_instance['query_tax_clauses'] ) )
			? $this->sanitize_advanced_taxonomy_clauses( $new_instance['query_tax_clauses'], $instance['post_type'] )
			: array();
	...
	/**
	 * Sanitize advanced taxonomy clauses for widget storage.
	 *
	 * @since 3.4.0
	 *
	 ...
	 * @return array
	 */
	private function sanitize_advanced_taxonomy_clauses( $clauses, $post_type ) {
		$clauses          = \Display_Post_Types\Frontend\Inc\Query_Schema::normalize_tax_clauses( $clauses );
		$valid_taxonomies = get_object_taxonomies( $post_type );
		if ( empty( $clauses ) || empty( $valid_taxonomies ) ) {
			return array();
		}

		$valid_clauses = array();
		foreach ( $clauses as $clause ) {
			if ( empty( $clause['taxonomy'] ) || ! in_array( $clause['taxonomy'], $valid_taxonomies, true ) ) {
				continue;
			}

			$terms = get_terms( array( 'taxonomy' => $clause['taxonomy'], 'hide_empty' => false ) );
			if ( is_wp_error( $terms ) ) {
				continue;
			}

			$valid_terms     = array_keys( wp_list_pluck( $terms, 'name', 'slug' ) );
			$clause['terms'] = array_values( array_intersect( $clause['terms'], $valid_terms ) );
			if ( empty( $clause['terms'] ) ) {
				continue;
			}

			$valid_clauses[] = $clause;
		}
	...
	public function advanced_taxonomy_control( $instance ) {
		...
		$markup  = sprintf(
			'<input class="dpt-query-tax-clauses-value" name="%1$s" id="%2$s" type="hidden" value="%3$s" />',
	...
	public function select( $for, $options, $selected, $classes = array(), $echo = true, $atts = '' ) {
		...
		$select = sprintf(
			'<select id="%1$s" name="%2$s" class="dpt-%3$s widefat" %5$s>%4$s</select>',
